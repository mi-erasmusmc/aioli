...
pub async fn dose_form(pool: &Pool) -> Result<(), Box<dyn Error>> {
    ...
    // let result = client.execute("update faers.drug_mapping_exact_1 set dose_form_clean = concat(' ', regexp_replace(dose_form, '[\\(\\)\\,\\.]', '', 'gi'), ' ')", &[]).await.unwrap();
    // println!("Rows {} alterd", result);
    //
    // let result = client.execute("update faers.drug_mapping_exact_1 set route_clean = concat(' ', regexp_replace(route, '[\\(\\)\\,\\.]', '', 'gi'), ' ')", &[]).await.unwrap();
    // println!("Rows {} alterd", result);

    let mut csv = Reader::from_path("reference.csv")?;

    for record in csv.deserialize() {
        let result: (String, String) = record?;
        println!("Replacing {} with {}", result.0, result.1);
        let query_1 = format!("update faers.drug_mapping_exact_1 set dose_form_clean = replace(dose_form_clean, '{}', '{}') where dose_form_clean like '% {} %'", result.0, result.1, result.0);
        let query_2 = format!("update faers.drug_mapping_exact_1 set route_clean = replace(route_form_clean, '{}', '{}') where route_clean like '% {} %'", result.0, result.1, result.0);
    ...
    let result = client.query("select distinct dose_form_clean from faers.drug_mapping_exact_1 where dose_form_clean is not null", &[]).await.unwrap();
    ...
    //   let query_1 = format!("update faers.drug_mapping_exact_1 set dose_form_clean = replace(dose_form_clean, '{}', '') where dose_form_clean like '% {} %'", d, d);
    //  let query_2 = format!("update faers.drug_mapping_exact_1 set route_clean = replace(route_clean, '{}', '') where dose_form_clean like '% {} %'", d, d);
    //   update(query_1, &client).await;

    client.execute("update faers.drug_mapping_exact_1 set dose_form_clean = null where dose_form_clean ~ '^[[:space:]]*$'", &[]).await?;
    client.execute("update faers.drug_mapping_exact_1 set route_clean = null where route_clean ~ '^[[:space:]]*$'", &[]).await?;

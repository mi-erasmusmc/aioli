...
WITH cte1 AS (SELECT DISTINCT lower(rx_ingredient) AS ing
     ...
     cte2 AS (SELECT DISTINCT r.rxcui AS rxcui, cte1.ing
              FROM cte1
                       JOIN rxnorm.rxnconso r ON cte1.ing = lower(r.str)
                  AND r.tty IN ('IN', 'MIN')
                  AND r.sab = 'RXNORM')
...
WITH cte1 AS (SELECT DISTINCT rxcui FROM drug_mapping_exact_java WHERE final_id IS NULL),
     cte2 AS (SELECT DISTINCT cte1.rxcui AS original, r2.rxcui AS rxcui
              FROM cte1
                       JOIN rxnorm.rxnrel rel ON cte1.rxcui = rel.rxcui1
                       JOIN rxnorm.rxnconso r_in ON rel.rxcui2 = r_in.rxcui
                       JOIN rxnorm.rxnrel rel2 ON rel.rxcui2 = rel2.rxcui1
                       JOIN rxnorm.rxnconso r2 ON r2.rxcui = rel2.rxcui2
                  AND r_in.tty = 'SBDF'
                  AND r_in.sab = 'RXNORM'
                  AND r2.tty = 'SCDF'),
     cte3 AS (SELECT DISTINCT cte2.original                                                 AS rxcui,
              ...
              FROM cte2
                       JOIN rxnorm.rxnrel rel ON rel.rxcui1 = cte2.rxcui
                       JOIN rxnorm.rxnconso r_in ON rel.rxcui2 = r_in.rxcui
                  AND r_in.tty IN ('IN', 'MIN')
                  AND r_in.sab = 'RXNORM'
